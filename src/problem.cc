...
bool Problem::IsPoseVertex(const std::shared_ptr<backend::Vertex>& v) {
    string type = v->TypeInfo();
    return type == string("VertexPose") ||
           type == string("VertexSpeedBias");
}

bool Problem::IsLandmarkVertex(const std::shared_ptr<backend::Vertex>& v) {
    string type = v->TypeInfo();
...
bool Problem::AddEdge(const shared_ptr<Edge>& edge) {
    ...
    for (auto &vertex: edge->Verticies()) {
        vertexToEdge_.insert(std::pair<ulong, shared_ptr<Edge> >(vertex->Id(), edge));
    }
    return true;
}
